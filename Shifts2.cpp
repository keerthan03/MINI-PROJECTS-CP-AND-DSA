...
string shifts(string s,int n,int k,int flag){
    ...
    for(int i=0;i<n;i++)
    {  if((int)s[i]>='A' && (int)s[i]<='Z'){ upper=90;lower=65;}
       else{ upper=122; lower=97;}
        if(((int)s[i]+k)>upper)//IF ASCII EXCEEDS 122 THEN SHIFT BACK TO  97 BY SUBTRACTING 26
            res+=s[i]-26+k;
        else if(((int)s[i]+k)<lower)//IF ASCII FALL SHORT OF 97 THEN SHIFT BACK TO 122 BY ADDING 26
            res+=s[i]+26+k;
        else res+=s[i]+k;
    }
    return res;
